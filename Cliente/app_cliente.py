...
class Cliente:
    ...
    def solicitar_rotas(self):
        ...
        self.__client_socket.sendall(json.dumps(requisicao).encode())
        rotas = json.loads(self.__client_socket.recv(1024).decode())
        return rotas
    ...
    def comprar_passagem(self, origem, destino, assento):
        ...
        resposta = json.loads(self.__client_socket.recv(1024).decode())
        if resposta["status"] == "sucesso":
            print(resposta["mensagem"])
            print("Assentos disponíveis: ", resposta["assentos_disponiveis"])

            requisicao_assento = {"assento": assento}
            self.__client_socket.sendall(json.dumps(requisicao_assento).encode())
            
            resposta_final = json.loads(self.__client_socket.recv(1024).decode())
